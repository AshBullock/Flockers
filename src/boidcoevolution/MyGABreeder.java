...
extends BreederBase {
	...
	public Integer num_injection = 0;
	public IChromosome mutant = null;
	public Integer Mutant_count = 0;
	public RuleDecoding Marker;

	public MyGABreeder(Hashtable<String,Integer> parameters) {
		super();
		Marker = new RuleDecoding(parameters);
	}
	...
	public Population evolve(Population a_pop, Configuration a_conf) {
		...
		if(num_injection!=0) {
			for(int j=0; j<num_injection; j++) {
				...
				System.out.println("--------- Mutant injection--------- ");
			}		
		}

		Mutant_count = 0;
		for(int bb = 0; bb<pop.size(); bb++) {		
			IChromosome Chromosome_pop = pop.getChromosome(bb);
			//mutant.setFitnessValueDirectly(Chromosome_pop.getFitnessValue());				
			if(Chromosome_pop.hashCode() == mutant.hashCode()) {
				Mutant_count++;
			}					
		}
		System.out.println("MyGABreeder.java:  Mutant_count is .........: " + Mutant_count);		
		...
		for(int j=0; j<pop.size(); j++) {
			...
			//System.out.println("2nd gene is .........: " + individual.getGene(2));	
