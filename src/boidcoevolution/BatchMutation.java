...
/**
 * This new mutation operator runs through the genes in each of the Chromosomes
 * in the population and mutates them in statistical accordance to the
 * given mutation rate. One randomly selected mutated Chromosome is then 
 * cloned for and replaced 50% of the worst genes in the list of
...
extends BaseGeneticOperator implements Configurable {
	 ...
	 * @since 3.0
	 */
	public BatchMutation(final Configuration a_conf)
	throws InvalidConfigurationException {
		super(a_conf);
	 ...
	 * @since 1.1
	 */
	public BatchMutation(final Configuration a_config,
	...
	public BatchMutation(final Configuration a_config,
			final int a_desiredMutationRate, final double a_desiredCloneRate)
	throws InvalidConfigurationException {
		super(a_config);
		m_config.m_mutationRate = a_desiredMutationRate;
		setMutationRateCalc(null);
	...
	public void operate(final Population a_population,
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < genes.length; j++) {
				...
				if (m_mutationRateCalc != null) {
					...
					{
						if(j==genes.length-1)
							mutate = (generator.nextInt(100) == 0);
						else
							mutate = (generator.nextInt(500) == 0);
					}
					else
				...
				if (mutate) {
					...
					if (genes[j] instanceof ICompositeGene) {
						...
						//System.out.println("j " + j + " genes[j]" + genes[j]);
		...
		// Sort the a_population according to their fitness
		// Now the size of a_population is 2*a_population before 
		// mutation.
		// Because the population size is constant, eventually  
		// JGAP will cut off the 50% of the lower chromosomes
		// --------------------------------------------------------------
	...
	public int compareTo(Object a_other) {
		...
		BatchMutation op = (BatchMutation) a_other;
		if (m_mutationRateCalc == null) {
